...
export const Documents = () => {
  ...
  const getStatusBadge = (status: Document['status']) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${statusConfig[status]}`}
      >
  ...
  const StatCard = ({ icon: Icon, label, value, color, bgColor }) => (
    <div
      className={`${bgColor} rounded-2xl p-6 border border-gray-100 hover:shadow-lg transition-all duration-300`}
    >
            ...
            <div className='flex justify-between items-center'>
              <div>
                ...
                <p className='text-gray-600'>
                  Add multiple documents at once with automatic processing
                </p>
              </div>
              <button
              ...
              <div className='flex items-start gap-3'>
                ...
                <div>
                  ...
                  <p className='text-sm text-blue-800'>
                    Upload clear, high-quality scans. Accepted formats: PDF,
                    JPG, PNG. Maximum file size:{' '}
                    {formatFileSize(FILE_SIZE_LIMIT)}. Some documents may
                    require MOFA attestation.
                  </p>
                </div>
              </div>
            </div>
